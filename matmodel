...
stages = [{'F_t': 700000, 'mass': 10500, 'time': 60},  # Первая ступень
          {'F_t': 330000, 'mass': 8000, 'time': 132}]  # Вторая ступень
...
# Расчет ускорения
acceleration = np.gradient(np.sqrt(x_speeds**2 + y_speeds**2), time)
...
plt.plot(data["pastime"][1:], data["velocity"][1:], color="red", label="Скорость (KSP)")
...
plt.plot(data["pastime"][1:], data["height"][1:], color="red", label="Высота (KSP)")
plt.ylabel("Высота, м")
plt.xlabel("Время, с")
plt.legend()
