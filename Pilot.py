...
import math                  # Нужна для математических функций (sqrt, exp, и т.д.)
...
# Часто в ракетах KSP с большим количеством ступеней нужно вызвать несколько раз,
# потому что в первой "секции" может быть запуск сасла (launch clamps) и т.д.
vessel.control.activate_next_stage()
...
# Когда цель достигнута, отделяем, например, обтекатель (защитный купол),
# предполагая, что он был в очередной ступени.
vessel.control.activate_next_stage()
...
# Перематываем время в игре (timewarp), чтобы не ждать зря
